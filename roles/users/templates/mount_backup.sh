...
# Mount SSHFS
sshfs "${sshfs_remote_user}@${sshfs_remote_host}:${sshfs_remote_dir}" "$sshfs_local_dir" -o IdentityFile="$sshfs_remote_user_key" -o uid="$(id -u $(whoami))" -o gid="$(id -g $(whoami))" -o port="${sshfs_remote_port}" &&
